<script lang="ts">
  import { PERFECT_SQUARES } from '$lib/constants';

  let search = $state('');
  let filtered = $derived(
    search ? PERFECT_SQUARES.filter(s => String(s.n).includes(search) || String(s.val).includes(search))
           : PERFECT_SQUARES
  );
</script>

<div class="section-label" style="margin-bottom:16px;">Perfect Squares Reference (1–50)</div>
<div class="table-wrap">
  <div class="table-header">
    ...
    <span style="font-family:var(--mono);font-size:0.72rem;color:var(--text-dim);margin-left:auto;">{filtered.length} / {PERFECT_SQUARES.length}</span>
  </div>
  <div class="table-scroll" style="max-height:600px;overflow-y:auto;">
    <table style="width:100%;border-collapse:collapse;font-family:var(--mono);font-size:0.78rem;">
      <thead><tr style="background:var(--surface2);">
        <th style="padding:10px 16px;text-align:left;border-bottom:1px solid var(--border);">n</th>
        <th style="padding:10px 16px;text-align:left;border-bottom:1px solid var(--border);">n²</th>
        <th style="padding:10px 16px;text-align:left;border-bottom:1px solid var(--border);">√(n²)</th>
      </tr></thead>
      <tbody>
        {#each filtered as s}
          <tr style="border-bottom:1px solid var(--border);">
            <td style="padding:8px 16px;color:var(--accent4);">{s.n}</td>
            <td style="padding:8px 16px;color:var(--accent);">{s.val}</td>
            <td style="padding:8px 16px;color:var(--text-mid);">√{s.val} = {s.n}</td>
          </tr>
        {/each}
      </tbody>
    </table>
