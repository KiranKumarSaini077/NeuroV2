...
if (-not $token) { Write-Host "ERROR: GITHUB_TOKEN missing in .env.local" -ForegroundColor Red; exit 1 }
...
& $git add -A
if ($msg) { $commitMsg = $msg } else { $commitMsg = "auto: update $(Get-Date -Format 'yyyy-MM-dd HH:mm')" }
& $git commit -m $commitMsg
if ($LASTEXITCODE -ne 0) { Write-Host "Nothing to commit" -ForegroundColor Yellow; exit 0 }
